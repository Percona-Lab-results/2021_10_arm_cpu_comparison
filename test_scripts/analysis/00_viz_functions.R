...
intel_graviton_comparison <- function(input_dt, 
  ...
  if(input_relative){
    ...
    title_generated <- "Relative (%) performance difference between Intel and Graviton (queries per second)"
    ...
    title_generated <- "Absolute performance difference between Intel and Graviton (queries per second)"
  ...
  result_plot <- result_plot+ 
    ...
    scale_fill_manual(name = 'CPU type', 
                      values =c('Graviton'='darkgoldenrod1','Intel'='dodgerblue'), labels = c('Graviton','Intel')) +
...
efficient_comparison_point_plot <- function(input_dt, facet_threads=FALSE){
  ...
  result_plot <- ggplot(temp_dt,aes(x=dollar_request,
                     ...
                     size=as.factor(Number_of_threads))) +
    geom_point() +
    labs(title = "CPU types efficient comparison ",
